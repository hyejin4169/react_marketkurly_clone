...
const DetailItem = (props) => {
    ...
    return (
        ...
        <SectionView>
                ...
                <TitleWrap>
...

                    {
                        (data.discountPercent == 0) ?
                            ...
                            <PriceWrap>
                                <p>회원할인가</p>
                                <Price>{Number(data.discountedPrice).toLocaleString()}<span>원</span></Price>
                                <Disconunt>{data.discountPercent}%</Disconunt>
                                <Original>{Number(price).toLocaleString()}<span>원</span></Original>
                            </PriceWrap>
                    ...
                    <InfoWrap>
                        ...
                        <dl className="list">
                            ...
                            <dd className="desc">
                                해당제품은 최저가로 정상가 세팅되었으나,  유통사(수입사)의 가격정책이슈로 정상 판매가로 재세팅하였습니다. (단, 이전가격보다 저렴한 가격으로 할인판매예정/12월말까지)
                            </dd>
                        </dl>
                    </InfoWrap>
                </TitleWrap>
                ...
                <BoxSelect>
                    ...
                    <Option>
                        <span className="count">
                            <button className="down btn" onClick={downCount} disabled={count < 2}></button>
                            <input type="number" className="inp" onChange={value} value={count}></input>
            ...
            <BtnWrap>
                <button className="btn" onClick={addCart}>장바구니 담기</button>
            </BtnWrap>
        </SectionView>
    );
};
...
const Option = styled.div`
    ...
    .up {
        float: right;
        margin-left: -1px;
        background: url("https://res.kurly.com/pc/ico/2010/ico_plus_on.svg") no-repeat 50% 50%;
